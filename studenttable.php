...
<input type="text" id="filterInput" placeholder="Search..." style="     text-align: center;
    ...
    font-size: 34px;
    font-weight: 300;
    margin-top: -59px;">Students list</h5>
    <thead>
          <tr>
            <th>S.no</th>
            <th>Name</th>
...
http://localhost/musicon/landing/create.php
      ...
      <script>
        ...
        function filterTable() {
            ...
            tableRows.forEach((row) => {
                ...
                const batches = row.querySelector('td:nth-child(4)').textContent.toLowerCase();

                if (number.includes(filterText) || name.includes(filterText) || batch.includes(filterText) || batches.includes(filterText)) {
                    row.style.display = '';
                } else {
      ...
      <button class="btnOpen">Add new Student</button>
      <section class="box">
          ...
          <form action="studenttablemain.php" method="POST" enctype="multipart/form-data" >
          <div class="registration-box">
              <div>
                  <input type="text" 
                   name="number" required="required" style="height:47px">
                  <label>Number</label>
               </div>
      
               <div>
                  <input type="text"
                    name="name" required="required"  style="height:47px">
                  <label>Name</label>
               </div>
               <div>
             <input type="text" name="batch" required  style="height:47px">
                 <label>Courses</label>
               </div>
               <div>
             <input type="text" name="batches" required  style="height:47px">
                 <label>Batches</label>
               </div>
